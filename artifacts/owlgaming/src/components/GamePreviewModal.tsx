...
export default function GamePreviewModal({ game, onClose }: GamePreviewModalProps) {
  ...
  const handleToggleCart = () => {
    ...
    else addItem({ id: game.id, title: game.title, size: game.size, price: roundUpGameSizeGB(game.size) * pricing.pricePerGb, image: game.image });
  };
      ...
      >
        ...
        <div className="px-5 pb-5 space-y-4 -mt-1">
          ...
          {(() => {
            ...
            return (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm pt-1 border-t border-border">
                {devs.length ? (
                  <div>
