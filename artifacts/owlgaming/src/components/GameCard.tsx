...
function GameCardImpl({ id, title, image, size, downloads, price, source, genre }: GameCardProps) {
      ...
      >
        ...
        >
          ...
          <div className="relative w-full overflow-hidden bg-[#0d0f1a]" style={{ aspectRatio: "16/9" }}>
            ...
            {/* Size badge — top-left */}
            <div className="absolute top-2 left-2 px-2 py-0.5 rounded-md bg-black/85 border border-white/15">
            ...
            <div className="absolute top-2 right-2 flex gap-1">
              <button
                ...
                className={`w-9 h-9 rounded-lg border flex items-center justify-center transition-colors duration-150 ${
                  fav
                    ? "bg-primary/30 border-primary/70"
                    : "bg-black/75 border-white/15 hover:border-primary/50"
                } ${favAnimating ? "scale-125" : ""}`}
                aria-label={fav ? "Remove from favorites" : "Add to favorites"}
              >
                <Heart className={`w-4 h-4 transition-all ${fav ? "text-primary fill-primary" : "text-white"}`} />
              </button>
              <button
                onClick={handlePreview}
                className="w-9 h-9 rounded-lg bg-black/75 border border-white/15 flex items-center justify-center hover:bg-primary/50 hover:border-primary/60 transition-colors"
                aria-label="Preview"
              >
          ...
          <div className="bg-card border border-t-0 border-border rounded-b-xl px-2.5 pt-2 pb-2.5 flex flex-col gap-2">
            {genre && (
              <span className="inline-block self-start px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider bg-primary/15 text-primary/80 border border-primary/25">
                {genre}
              </span>
            )}
