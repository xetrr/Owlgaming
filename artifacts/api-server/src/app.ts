...
// Reject state-changing requests whose Origin header doesn't match the host.
// This provides CSRF defence in production without requiring token round-trips.
app.use((req, res, next) => {
  if (!isProduction) return next();
  const method = req.method.toUpperCase();
  if (method === "GET" || method === "HEAD" || method === "OPTIONS") return next();
  const origin = req.headers["origin"];
  if (!origin) return next(); // same-origin requests from non-browser clients have no Origin
  try {
    const reqHost = (req.headers["x-forwarded-host"] as string) || req.headers.host || "";
    const originHost = new URL(origin).host;
...
app.use(
  session({
    secret: sessionSecret || "owlgaming-dev-secret-do-not-use-in-prod",
    resave: false,
